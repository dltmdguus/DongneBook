...
function page(n){
	...
	location.href="./?pageNo="+pageNo+"&searchN=${param.searchN}&searchV=${param.searchV}";
	}
}
function login(){
...
<body data-spy="scroll" data-target=".onpage-navigation" data-offset="60">
 <%@ include file="./menu.jsp"%>
    <main>
      <div class="main">
       <hr class="divider-w">
         <div class="container" style="width: 100%; margin-bottom: 50px; margin-top:50px;">
 			<!--  옆 메뉴바  -->
 			
 			<div class="col-sm-1 col-md-1 sidebar " style= "margin-top:400px;">
               <div class="widget">
           ...
           <!--  옆 메뉴바  -->
 			...
 			<div class="col-sm-8 col-sm-offset-1" style="margin-left: 165px;">
 			 <div class="fi"><img src="../board/커뮤니티.jpg" style="width: 600px;  height : 300px" class="rounded">
                <h1>전체글</h1></div>
            <div class="container" >
              <div class="first">
             
               	<ul class="filter font-alt" >
               <c:choose> <c:when test="${sessionScope.mid ne null}"><button type="button" class ="btn btn-border-d btn-round firstb" onclick="location.href='./cwrite'">글쓰기</button> &nbsp; &nbsp;</c:when>
               <c:otherwise><button type="button"  class ="btn btn-border-d btn-round firstb" onclick="login()">글쓰기</button>&nbsp; &nbsp; </c:otherwise></c:choose>
               	<c:forEach items="${boardList }" var="l"><il><a href="${l.surl }">${l.stitle }</a></il> &nbsp; &nbsp; </c:forEach> <il><a href="./">전체</a></il>
                &nbsp; &nbsp;<il> ${pag.pageNo} /${pag.frontpg } 페이지</il>
                </ul>
               </div>
          ...
          <!--   검색창  -->
               ...
               <thead>
               	<tr class="row">
               		...
               		<th class="col-2 bb">카테고리</th>
               		<th class="col-1 bb">읽음</th>
               	</tr>
               </thead>
               <tbody><c:forEach items="${list}" var="row">
               <tr class="row detail" onclick="location.href='./bdetail?board=${row.bcate}&bno=${row.bno}'">
               		<td class="col-6 title">${row.btitle } &nbsp;<c:if test="${row.commentcount  ne 0}"><img src="../board/댓글.png" style="width: 10px;  height : 10px" class="rounded"> ${row.commentcount} </c:if></td>
               		<td class="col-2 bb">${row.bdate }</td>
               		<td class="col-1 bb">${row.mid }</td>
               		<td class="col-2 bb">${row.stitle }</td>
